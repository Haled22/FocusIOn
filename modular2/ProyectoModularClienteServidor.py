...
import requests
#Clase que se encarga de la distribución la logica de decision en un modelo cliente servidor
#Manda información a el servidor para toma de decisiones
class Servidor:
    def __init__(self, server_url):
        self.server_url = server_url

    def mandar_datos(self, x, y):
        try:
...
class FocusIOn(QMainWindow):   
    ...
    def evt_clic(self):
       ...
       if area_trabajo and self.tarea_iniciada:
            ...
            x = QCursor.pos().x()
            y= QCursor.pos().y()
            result = self.tracking_client.mandar_datos(
        x, y
    )

            status = result.get("status")
                   
            if str(status) == "No distraido":
